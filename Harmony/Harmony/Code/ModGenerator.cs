...
namespace Harmony
{
    public static class ModGenerator
    {
        ...
        private static async Task GenerateNarration(Language langId, Character charId, bool silent)
        {
            ...
            using Stream stream = await DownloadManager.DownloadAsset(Paths.GitHub.Narration, Paths.Working);
            using ZipArchive zip = new(stream);

			string unpackDir = Paths.Generator.Temp;
            Directory.CreateDirectory(unpackDir);

            // Unpack the asset bundle into a temporary directory
            await Task.Run(() => zip.ExtractToDirectory(unpackDir));

            // Either rename or delete the silent audio files depending on the user's choice
            await Task.Run(() => ProcessEmptyAudio(unpackDir, silent));

            // Generate the mod with UnrealPak
            await Task.Run(() => CreatePak(unpackDir, unpackDir));
            ...
            File.Move(Paths.Generator.Narration, $"{modFileName}.pak", true);
        ...
        private static async Task GenerateIndividual(PatchInfo patch, Action iterationCallback)
        {
			...
			//{
			//	Paths.Character	= langPatch.Character;
			//	Paths.Language	= langPatch.UseLang;

			//	using Stream stream	= null;// await	DownloadManager.DownloadAsset(Paths.Assets.Voice.URL, Paths.Assets.Voice.Cache);
			...
			//	await Task.Run(() => RecursiveRename(unpackDir,	langPatch.UseLang.ToString(), langPatch.OverLang.ToString()));

			//	// Generate	the	mod	with UnrealPak
			//	await Task.Run(() => CreatePak(unpackDir, Paths.Generator.Voice));
			...
			//	File.Move(Paths.Generator.Voice, $"{modFileName}.pak", true);
			//	File.Copy(Paths.Config.GameSig,	 $"{modFileName}.sig", true);
